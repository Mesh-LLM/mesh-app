...
/// Automatic recipes request 64K only when the user has no context override.
/// Unknown or malformed config is left to the engine, never overridden.
pub fn automatic_context_args(profile: &Path) -> Vec<String> {
    let Ok(text) = std::fs::read_to_string(profile.join("config.toml")) else {
        return Vec::new();
    };
    context_args(&text)
...
fn context_args(text: &str) -> Vec<String> {
    fn has_context(value: &toml::Value) -> bool {
        match value {
            toml::Value::Table(table) => {
                table.contains_key("ctx_size") || table.values().any(has_context)
            }
            toml::Value::Array(array) => array.iter().any(has_context),
            _ => false,
        }
    }
    match toml::from_str::<toml::Value>(text) {
        Ok(value) if value.get("models").is_none() && !has_context(&value) => {
            vec!["--ctx-size".into(), "65536".into()]
        }
        _ => Vec::new(),
    }
}
...
mod context_tests {
    use super::*;
    #[test]
    fn automatic_context_never_overrides_user_context_or_models() {
        assert_eq!(context_args(""), ["--ctx-size", "65536"]);
        for config in [
            "[defaults.model_fit]\nctx_size=131072",
            "[defaults]\nctx_size=0",
            "[[models]]\nmodel='custom'",
            ...
            assert!(context_args(config).is_empty());
        }
    }
}
