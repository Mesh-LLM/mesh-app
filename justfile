...
console-build:
    #!/usr/bin/env bash
    set -euo pipefail
    ui=$(rustup run stable cargo metadata --format-version 1 | python3 -c 'import json,sys,pathlib; print(pathlib.Path(next(p["manifest_path"] for p in json.load(sys.stdin)["packages"] if p["name"] == "mesh-llm-ui")).parent)')
